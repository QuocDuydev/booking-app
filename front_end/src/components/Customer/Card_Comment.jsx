...
export function CardComments({ users, limitedRecomment }) {
	return (
		<div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 p-2 sm:flex sm:justify-center">
					...
					>
						<Avatar
							...
							src={
								users.length > 0 &&
								item.user &&
								users.find((user) => user.data.id === item.user)?.data.images
							}
						/>
						<div className="flex w-full flex-col gap-0.5">
							<div className="flex items-center justify-between">
								...
								<div className="5 flex items-center gap-0 text-xs">
									{item.rating && (
										<>
											<Rating
												value={item.rating}
