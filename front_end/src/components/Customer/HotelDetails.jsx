...
export default function HotelDetails() {
		...
		const fetchRooms = async () => {
			try {
				...
				if (roomData.length > 0) {
					...
					const amenitiesData = await getAmenitiesByRoomId(roomId);
	...
	const getAmenitiesByRoomId = async (roomId) => {
		try {
			const response = await axios.get(
				"http://localhost:8000/api/room-amenities/",
				{
					headers: {
			...
			// Lọc danh sách amenities theo roomId
			const amenitiesByRoom = response.data.filter(
				(amenity) => amenity.rooms === roomId,
			);

			// Cập nhật state amenities
			setAmenities(amenitiesByRoom);

			return amenitiesByRoom;
		} catch (error) {
			console.error(
				"Error fetching amenities data for room ID",
				roomId,
				":",
				error,
			);
	...
	const getRoomsByAccommodationId = async () => {
		try {
			const response = await axios.get(
				`http://localhost:8000/api/accommodations/${acc_id}/rooms/`,
				{
					headers: {
						Authorization: `Bearer ${token}`,
					},
			...
			<Card className=" w-full mx-auto mb-2 shadow-none mt-12">
				...
				<CardBody>
					<div className="flex flex-wrap container mx-auto">
						...
						<div className="flex flex-col flex-wrap px-3 py-3 2xl:w-1/3 lg:w-1/3  md:mx-auto md:w-1/2 sm:w-1/2  sm:mx-auto  lg:ml-3 lg:-mr-3 2xl:ml-3 2xl:-mr-3 border border-solid border-gray-400 rounded-lg mb-3">
